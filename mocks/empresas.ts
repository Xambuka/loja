...
export function verificarHorarioFuncionamento(empresaId: string): boolean {
  ...
  const diasDaSemana = ['domingo', 'segunda', 'terca', 'quarta', 'quinta', 'sexta', 'sabado'] as const;
  const diaSemana = agora.getDay(); // 0 a 6

  const diaNome = diasDaSemana[diaSemana];
  if (!diaNome) return false;
  ...
  const abertoAgora = horariosDoDia.some(intervalo => {
    ...
    if (partes.length !== 2) return false; // invalido, ignora esse intervalo
...
export function getConfiguracaoEmpresa(empresaId: string) {
