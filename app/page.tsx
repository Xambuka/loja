...
import { getAnunciosAtivos } from '../mocks/anuncios';
      ...
      <section className="container mx-auto px-4 mb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {empresasDestaque.map((empresa, index) => {
            // Asserção de tipo completa para o objeto empresa
            const empresaTipada = empresa as unknown as Empresa;
            return (
              <EmpresaCard 
                key={index} 
                empresa={empresaTipada} 
                destaque={true} 
              />
            );
          })}
      ...
      <section className="container mx-auto px-4 mb-12">
        <div className="relative h-48 md:h-64 w-full rounded-lg overflow-hidden">
          <img 
            ...
            alt={anunciosDestaque[0]?.titulo || 'Anúncio'}
            className="w-full h-full object-cover"
          />
          <div className="absolute inset-0 bg-gradient-to-r from-black/60 to-transparent flex items-center">
            <div className="p-6 md:p-10 max-w-lg">
              <h3 className="text-2xl md:text-3xl font-bold text-white mb-2">
                {anunciosDestaque[0]?.titulo || 'Título do anúncio'}
              </h3>
              <p className="text-white/90 mb-4">
                {anunciosDestaque[0]?.descricao || 'Descrição do anúncio.'}
              </p>
              <Link 
                href={`/empresa/${anunciosDestaque[0]?.empresaId || '1'}`}
